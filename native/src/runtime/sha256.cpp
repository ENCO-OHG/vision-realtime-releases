...
std::string sha256Hex(const std::string& input) {
    ...
    for (std::size_t offset = 0; offset < message.size(); offset += 64) {
        ...
        for (int i = 0; i < 16; ++i) words[i] = (static_cast<std::uint32_t>(message[offset + i * 4]) << 24) | (static_cast<std::uint32_t>(message[offset + i * 4 + 1]) << 16) | (static_cast<std::uint32_t>(message[offset + i * 4 + 2]) << 8) | message[offset + i * 4 + 3];
        for (int i = 16; i < 64; ++i) {
        ...
        for (int i = 0; i < 64; ++i) {
            ...
            const auto temp1 = h + s1 + choose + K[i] + words[i];
    ...
    for (const auto value : hash) result << std::hex << std::setfill('0') << std::setw(8) << value;
    return result.str();
}
